...
class TableView(ModelView):
    can_delete = False
    form_columns = ["tablename", "table_people", "restaurant"]
    column_list = ["tablename", "table_people", "restaurant"]
...
admin.add_view(TableView(RestTable, db.session))
...
app.config['SECRET_KEY'] = 'ge8e9wgub'
...
def cold_drink():
    food = db.session.query(Food).all()
    return render_template('coldD.html', food=food)

@app.route('/hot_fod')
def hot_food():
    food = db.session.query(Food).all()
    return render_template('hotF.html', food=food)

@app.route('/snacks')
def snack():
    food = db.session.query(Food).all()
    return render_template('snack.html', food=food)

@app.route('/Light_Bites')
def small():
    food = db.session.query(Food).all()
    return render_template('lightBite.html', food=food)

@app.route('/Book_Table')
def table_booking():
    table = db.session.query(Food).all()
    return render_template('tables.html', table=table)

@app.route('/Leeds_Booking')
def leeds():
    table = db.session.query(RestTable).all()
    return render_template('leeds.html', table=table)
